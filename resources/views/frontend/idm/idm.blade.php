            ...
            <div class="row">
                <div class="col col1" data-aos="fade-right">
                    <div class="container-fluid">
                        <h2 class="text-center"><b>INDEKS DESA MEMBANGUN (IDM)</b></h2>
                        {{-- {{$res['features']}} --}}
...
@push('javascript')
    ...
    <script>
        ...
        function getColorDB(d) {
            return (d) == 'SANGAT TERTINGGAL' ? 'red' :
                (d) == 'TERTINGGAL' ? 'chocolate' :
                (d) == 'BERKEMBANG' ? 'yellow' :
                (d) == 'MAJU' ? 'green' :
                (d) == 'MANDIRI' ? 'blue' :
                'red';
        ...
        $.getJSON(wfs_url_adm).then((res) =>{
            var layer = L.geoJson(res, {
                onEachFeature: function(f,l) {
                    ...
                    var idm = {!! json_encode($idm) !!};
                    for (let i = 0; i < idm.length; i++) {
                        // console.log(sites[i].kode_desa);
                        if (idm[i].desa_id == f.properties.kode_desa) {
                            Id = idm[i].id;
                            NilaiIdm = idm[i].nilai_idm;
                            StatusIdm = idm[i].status_idm;
                            Tahun = idm[i].tahun;
                        }
                    }
                    ...
                    var isPopup =
                            ...
                            "</li><li class='list-group-item'>Nilai IDM : " + NilaiIdm +
                            "</li><li class='list-group-item'>Status : " + StatusIdm +
                            "</li><li class='list-group-item'>Tahun : " + Tahun 
                            
                            // "</li><li class='list-group-item'><a href='idm/" +
                            // Id + "' 'type='button' class='btn btn-outline-primary btn-sm btn-block'>Detail</a></li>";
                ...
                style : function(feature){
                    ...
                    for (let i = 0; i < idm.length; i++) {
                    // console.log(sites[i].kode_desa);
                    if (idm[i].desa_id == feature.properties.kode_desa) {
                        StatusIdm = idm[i].status_idm;
                   ...
                   return { 
                    opacity: 0.9,
                    color: getColorDB(StatusIdm),
                    fillColor: getColorDB(StatusIdm)
                    // color: getColor(feature.properties.status_idm),
                    // fillColor: getColor(feature.properties.status_idm)
                }
                    }
        ...
        // }
            ...
            var legend1 = L.control({ position: "bottomright" });
                ...
                div.innerHTML += "<hr>";
                div.innerHTML += "<h4>Status IDM</h4>";
                div.innerHTML += '<i style="background: RED"></i><span>SANGAT TERTINGGAL</span><br>';
                div.innerHTML += '<i style="background: CHOCOLATE"></i><span>TERTINGGAL</span><br>';
                div.innerHTML += '<i style="background: YELLOW"></i><span>BERKEMBANG</span><br>';
                div.innerHTML += '<i style="background: GREEN"></i><span>MAJU</span><br>';
                div.innerHTML += '<i style="background: BLUE"></i><span>MANDIRI</span><br>';
                ...
                const legend = L.control.Legend({
                    ...
                    legends: [{
                        ...
                        type: "image",
                        layers: [IDM], 
                        url : "https://cdn-icons-png.flaticon.com/512/3281/3281306.png",
                        inactive: true,
                    }, {
                        label: "Marker",
                        type: "image",
